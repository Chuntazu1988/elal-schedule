...
def build_next_task_labels(result_df, employees_df):
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        # Night shift override: allow break right after first task if it ends by 01:30
        # (task_end_m >= 22:00 means task ended late evening; <= 90 means ended by 01:30)
        if shift_type == "night" and stage == 0 and break_label:
            in_night_early_window = (task_end_m >= 22 * 60) or (task_end_m <= 1 * 60 + 30)
            if in_night_early_window:
                can_break_now = True
