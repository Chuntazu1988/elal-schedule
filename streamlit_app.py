...
if "schedule_df" in st.session_state:
    try:
        ...
        with tab_gantt:
            st.subheader("📅 גאנט טיסות — אינטראקטיבי")
            st.caption("גרור עובד ממשבצת אחת לטיסה אחרת. השינוי יתעדכן בלוח המבצעים.")
            ...
            if timed.empty:
                ...
                all_starts = pd.to_datetime(timed["התחלה"], format="%H:%M", errors="coerce")
                all_ends   = pd.to_datetime(timed["סיום"],   format="%H:%M", errors="coerce")
                day_min = max(int(all_starts.dt.hour.min()) - 1, 0)
                day_max = min(int(all_ends.dt.hour.max()) + 2, 24)

                flights_sorted = safe_sort_by_time(live_flights, "המראה")["טיסה"].tolist()

                # Build JSON data for the JS component
                import json
                flights_data = []
                for fnum in flights_sorted:
                    dest_row = live_flights[live_flights["טיסה"] == fnum]
                    dest = dest_row.iloc[0]["יעד"] if not dest_row.empty else ""
                    dep  = dest_row.iloc[0]["המראה"] if not dest_row.empty else ""
                    ftasks = timed[timed["טיסה"].astype(str).str.strip() == fnum.strip()]
                    tasks_list = []
                    for idx, task in ftasks.iterrows():
                        tasks_list.append({
                            ...
                            "color":  ROLE_COLORS.get(normalize_role_label(str(task.get("תפקיד בסיס", ""))), "#9fb7d7"),
                        })
                    flights_data.append({
                        "flight": fnum,
                ...
                gantt_json = json.dumps(flights_data, ensure_ascii=False)
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ font-family: Arial, "Noto Sans Hebrew", sans-serif; background: #f8fafc; }}
  #gantt-wrap {{ overflow-x: auto; border: 1px solid #d9e2ef; border-radius: 12px; background: #fff; }}
  #gantt-svg {{ display: block; }}
  .flight-row {{ cursor: default; }}
  .task-block {{ cursor: grab; transition: opacity 0.15s; }}
  .task-block:hover {{ opacity: 0.75; }}
  .task-block.dragging {{ opacity: 0.4; cursor: grabbing; }}
  #drop-hint {{
    position: fixed; bottom: 18px; left: 50%; transform: translateX(-50%);
    background: #071b3a; color: #fff; padding: 8px 20px; border-radius: 20px;
    font-size: 13px; font-weight: 800; display: none; z-index: 999;
  ...
  #legend {{
    display: flex; flex-wrap: wrap; gap: 10px; margin: 10px 6px; direction: rtl;
  }}
  .legend-item {{
    display: flex; align-items: center; gap: 5px;
    font-size: 12px; font-weight: 800; color: #071b3a;
  }}
  .legend-dot {{
...
<div id="drop-hint">🔄 שחרר כדי להחליף עובד</div>
<div id="swap-result"></div>

<script>
const flights = {gantt_json};
...
const SVG_W = LEFT_PAD + (DAY_MAX - DAY_MIN) * HOUR_W + RIGHT_PAD;
const NS = "http://www.w3.org/2000/svg";

let dragging = null;  // {{ taskIdx, flightIdx, taskObj, element }}
let swapLog = [];

function hToX(h) {{
  const [hh, mm] = h.split(":").map(Number);
  return LEFT_PAD + (hh + mm / 60 - DAY_MIN) * HOUR_W;
}}
...
function assignLanes(tasks) {{
  ...
  return tasks.map(t => {{
    ...
    for (let li = 0; li < lanes.length; li++) {{
      if (lanes[li].every(([ls, le]) => s >= le || e <= ls)) {{
        lanes[li].push([s, e]);
        placed = li;
        break;
      }}
    }}
    if (placed === -1) {{ lanes.push([[s, e]]); placed = lanes.length - 1; }}
...
function buildSVG() {{
  ...
  // Compute heights
  const flightMeta = flights.map(f => {{
    const assigned = assignLanes(f.tasks);
    const nLanes = assigned.length ? Math.max(...assigned.map(t => t.lane)) + 1 : 1;
    return {{ ...f, assigned, nLanes, rowH: nLanes * (LANE_H + LANE_PAD) + 14 }};
  }});

  const totalH = HEADER_H + flightMeta.reduce((s, f) => s + f.rowH, 0) + 10;
  ...
  // Background
  svg.appendChild(el("rect", {{ width: SVG_W, height: totalH, fill: "#f8fafc" }}));

  // Hour grid
  for (let h = 0; h <= DAY_MAX - DAY_MIN; h++) {{
    const x = LEFT_PAD + h * HOUR_W;
    const hr = String(DAY_MIN + h).padStart(2, "0") + ":00";
    svg.appendChild(el("line", {{ x1: x, y1: HEADER_H - 4, x2: x, y2: totalH, stroke: "#dde3ed", "stroke-width": 1 }}));
    svg.appendChild(el("text", {{ x: x + 3, y: HEADER_H - 10, "font-size": 11, fill: "#6b7fa3" }}, hr));
  }}

  // Flight rows
  let yCursor = HEADER_H;
  flightMeta.forEach((f, fi) => {{
    const bg = fi % 2 === 0 ? "#ffffff" : "#f1f5fb";
    svg.appendChild(el("rect", {{ x: 0, y: yCursor, width: SVG_W, height: f.rowH, fill: bg }}));

    // Flight label
    const midY = yCursor + f.rowH / 2;
    svg.appendChild(el("text", {{ x: LEFT_PAD - 7, y: midY - 5, "font-size": 12, fill: "#071b3a", "font-weight": "bold", "text-anchor": "end" }}, f.short));
    svg.appendChild(el("text", {{ x: LEFT_PAD - 7, y: midY + 9, "font-size": 10, fill: "#6b7fa3", "text-anchor": "end" }}, f.dest));

    // Task blocks
    f.assigned.forEach((t, ti) => {{
      const x1 = LEFT_PAD + (t.sh - DAY_MIN) * HOUR_W;
      const bw = Math.max((t.eh - t.sh) * HOUR_W, 6);
      const ly = yCursor + 6 + t.lane * (LANE_H + LANE_PAD);
      const ty = ly + LANE_H / 2 + 1;

      const isMissing = t.worker.includes("❌");
      const blockColor = isMissing ? "#ffcccc" : t.color;
      const textColor  = isMissing ? "#9b0000" : "white";

      const clipId = `clip-${{fi}}-${{ti}}`;
      const clipEl = el("clipPath", {{ id: clipId }});
      clipEl.appendChild(el("rect", {{ x: x1 + 3, y: ly, width: Math.max(bw - 6, 0), height: LANE_H }}));
      svg.appendChild(clipEl);

      const rect = el("rect", {{
        x: x1, y: ly, width: bw, height: LANE_H,
        rx: 6, fill: blockColor, opacity: 0.9,
        "data-fi": fi, "data-ti": ti,
        "data-idx": t.idx,
        class: isMissing ? "task-block missing" : "task-block"
      }});

      const label = el("text", {{
        x: x1 + bw / 2, y: ty,
        "font-size": 10, fill: textColor, "font-weight": "bold",
        "text-anchor": "middle", "dominant-baseline": "middle",
        "clip-path": `url(#${{clipId}})`,
        "pointer-events": "none"
      }}, t.worker);

      const titleEl = el("title", {{}}, `${{t.worker}} | ${{t.role}} | ${{t.start}}–${{t.end}}`);
      rect.appendChild(titleEl);

      if (!isMissing) {{
        rect.addEventListener("mousedown", (e) => startDrag(e, rect, fi, ti, t, f));
        rect.addEventListener("touchstart", (e) => startDrag(e, rect, fi, ti, t, f), {{ passive: true }});
      }}

      svg.appendChild(rect);
      if (bw > 22) svg.appendChild(label);
    }});

    // Drop zone (whole row)
    const dropZone = el("rect", {{
      x: LEFT_PAD, y: yCursor, width: SVG_W - LEFT_PAD - RIGHT_PAD, height: f.rowH,
      fill: "transparent", "data-fi": fi,
      class: "drop-zone"
    }});
    dropZone.addEventListener("mouseup",   (e) => endDrag(e, fi, f));
    dropZone.addEventListener("touchend",  (e) => endDrag(e, fi, f));
    svg.appendChild(dropZone);

    // Row separator
    svg.appendChild(el("line", {{ x1: 0, y1: yCursor + f.rowH - 1, x2: SVG_W, y2: yCursor + f.rowH - 1, stroke: "#dde3ed", "stroke-width": 1 }}));
    yCursor += f.rowH;
  }});
}}

function startDrag(e, rect, fi, ti, task, flight) {{
  dragging = {{ fi, ti, task, flight, rect }};
  rect.classList.add("dragging");
  document.getElementById("drop-hint").style.display = "block";
  e.stopPropagation();
}}

function endDrag(e, targetFi, targetFlight) {{
  document.getElementById("drop-hint").style.display = "none";
  if (!dragging) return;

  const src = dragging;
  dragging.rect.classList.remove("dragging");
  dragging = null;

  if (src.fi === targetFi) return;  // same flight — no op

  // Find matching missing slot in target flight for same role
  const targetTasks = targetFlight.tasks;
  const missingSlot = targetTasks.find(t => t.worker.includes("❌") && t.role === src.task.role);

  const resultDiv = document.getElementById("swap-result");

  if (!missingSlot && !targetTasks.some(t => t.role === src.task.role)) {{
    resultDiv.style.display = "block";
    resultDiv.style.background = "#ffe8e8";
    resultDiv.style.color = "#9b0000";
    resultDiv.textContent = `❌ אין תפקיד מתאים (${{src.task.role}}) בטיסה ${{targetFlight.short}}`;
    return;
  }}

  // Send swap to Streamlit via postMessage
  const swapData = {{
    type: "gantt_swap",
    src_idx:    src.task.idx,
    src_worker: src.task.worker,
    src_flight: src.flight.flight,
    tgt_flight: targetFlight.flight,
    tgt_idx:    missingSlot ? missingSlot.idx : null,
    role:       src.task.role,
  }};

  swapLog.push(swapData);

  resultDiv.style.display = "block";
  resultDiv.style.background = "#d4edda";
  resultDiv.style.color = "#155724";
  resultDiv.textContent = `✅ ${{src.task.worker}} הועבר מ-${{src.flight.short}} ל-${{targetFlight.short}} (${{src.task.role}})`;

  // Update local state
  if (missingSlot) missingSlot.worker = src.task.worker;
  src.task.worker = "❌ חסר " + src.task.role;

  buildSVG();

  // Communicate to Streamlit
  window.parent.postMessage({{ isStreamlitMessage: true, type: "gantt_swap", data: swapData }}, "*");
}}
...
const legendRoles = [["ראש צוות","#8e24aa"],["דיילת","#5b9bd5"],["מתאם תורים","#f0a000"],["מפקח TSA","#d32f2f"],["שומר TSA","#2e7d32"],["טרייני ר״צ","#f9a825"]];
legendRoles.forEach(([r, c]) => {{
  const item = document.createElement("div");
  item.className = "legend-item";
  item.innerHTML = `<span class="legend-dot" style="background:${{c}}"></span><span>${{r}}</span>`;
...
"""
                ...
                components.html(gantt_html, height=max(600, len(flights_sorted) * 55 + 100), scrolling=True)

                st.info("💡 גרור משבצת עובד לשורת טיסה אחרת כדי להחליף. לאישור קבוע — השתמש בכפתור ההחלפה בלוח המבצעים.")


            st.subheader("📅 גאנט טיסות")
            st.caption("כל שורה = טיסה, ציר X = שעות היום. צבע לפי תפקיד העובד.")
            ...
            if timed.empty:
                ...
                search_gantt = st.text_input("🔎 חיפוש טיסה / יעד / עובד:", key="gantt_search")
                if search_gantt:
                    mask = timed.astype(str).apply(
                        lambda r: r.str.contains(search_gantt, case=False, na=False).any(), axis=1
                    )
                    matched_flights = timed[mask]["טיסה"].unique().tolist()
                    flights_sorted = [f for f in flights_sorted if f in matched_flights]

                LANE_H   = 26   # height of each lane inside a flight row
                LANE_PAD = 3    # padding between lanes
                LEFT_PAD = 100
                RIGHT_PAD = 20
                HOUR_W   = 70
                total_hours = day_max - day_min
                svg_w = LEFT_PAD + total_hours * HOUR_W + RIGHT_PAD

                # Pre-calculate lanes per flight to avoid overlaps
                def assign_lanes(tasks):
                    """Assign each task to a lane so no two tasks in the same lane overlap."""
                    lanes = []  # each lane = list of (start_h, end_h)
                    task_lanes = []
                    for _, task in tasks.iterrows():
                        try:
                            ts = pd.to_datetime(task["התחלה"], format="%H:%M")
                            te = pd.to_datetime(task["סיום"],   format="%H:%M")
                            sh = ts.hour + ts.minute / 60
                            eh = te.hour + te.minute / 60
                            if eh < sh:
                                eh += 24
                        except Exception:
                            task_lanes.append((0, 0, 0, task))
                            continue
                        placed = False
                        for li, lane in enumerate(lanes):
                            if all(sh >= le or eh <= ls for ls, le in lane):
                                lane.append((sh, eh))
                                task_lanes.append((li, sh, eh, task))
                                placed = True
                                break
                        if not placed:
                            lanes.append([(sh, eh)])
                            task_lanes.append((len(lanes)-1, sh, eh, task))
                    return task_lanes, len(lanes)

                # Calculate total SVG height
                flight_heights = {}
                flight_lane_data = {}
                for flight_num in flights_sorted:
                    flight_tasks = timed[timed["טיסה"].astype(str).str.strip() == flight_num.strip()]
                    task_lanes, num_lanes = assign_lanes(flight_tasks)
                    flight_lane_data[flight_num] = task_lanes
                    flight_heights[flight_num] = max(num_lanes, 1) * (LANE_H + LANE_PAD) + 10

                total_h = 36 + sum(flight_heights.values())
                svg_h = total_h + 10

                svg_lines = [
                    f'<svg xmlns="http://www.w3.org/2000/svg" width="{svg_w}" height="{svg_h}" '
                    f'style="font-family:Arial,sans-serif;direction:ltr;">',
                    f'<rect width="{svg_w}" height="{svg_h}" fill="#f8fafc"/>',
                ]

                # Hour grid + labels
                for h in range(total_hours + 1):
                    x = LEFT_PAD + h * HOUR_W
                    hour_label = f"{(day_min + h):02d}:00"
                    svg_lines.append(
                        f'<line x1="{x}" y1="28" x2="{x}" y2="{svg_h}" '
                        f'stroke="#dde3ed" stroke-width="1"/>'
                    )
                    svg_lines.append(
                        f'<text x="{x+3}" y="20" font-size="11" fill="#6b7fa3">{hour_label}</text>'
                    )

                y_cursor = 36
                for row_i, flight_num in enumerate(flights_sorted):
                    row_h = flight_heights[flight_num]
                    bg = "#ffffff" if row_i % 2 == 0 else "#f1f5fb"
                    svg_lines.append(
                        f'<rect x="0" y="{y_cursor}" width="{svg_w}" height="{row_h}" fill="{bg}"/>'
                    )

                    # Flight label
                    short_fnum = flight_num.replace("LY", "").strip()
                    dest_row = live_flights[live_flights["טיסה"] == flight_num]
                    dest = dest_row.iloc[0]["יעד"] if not dest_row.empty else ""
                    label_y = y_cursor + row_h // 2
                    svg_lines.append(
                        f'<text x="{LEFT_PAD - 6}" y="{label_y - 5}" font-size="12" '
                        f'fill="#071b3a" font-weight="bold" text-anchor="end">{html.escape(short_fnum)}</text>'
                    )
                    svg_lines.append(
                        f'<text x="{LEFT_PAD - 6}" y="{label_y + 9}" font-size="10" '
                        f'fill="#6b7fa3" text-anchor="end">{html.escape(dest)}</text>'
                    )

                    # Task blocks — each in its assigned lane
                    for (lane_i, sh, eh, task) in flight_lane_data[flight_num]:
                        try:
                            x1 = LEFT_PAD + (sh - day_min) * HOUR_W
                            x2 = LEFT_PAD + (eh - day_min) * HOUR_W
                            bar_w = max(x2 - x1, 6)

                            role  = normalize_role_label(task.get("תפקיד בסיס", ""))
                            color = ROLE_COLORS.get(role, "#9fb7d7")
                            worker_name = str(task.get("עובד", ""))

                            lane_y = y_cursor + 5 + lane_i * (LANE_H + LANE_PAD)
                            text_y = lane_y + LANE_H // 2 + 4

                            svg_lines.append(
                                f'<rect x="{x1:.1f}" y="{lane_y}" width="{bar_w:.1f}" '
                                f'height="{LANE_H}" rx="5" fill="{color}" opacity="0.88">'
                                f'<title>{html.escape(worker_name)} | {html.escape(role)}</title>'
                                f'</rect>'
                            )
                            # Full name inside block — clip with clipPath if needed
                            if bar_w > 20:
                                clip_id = f"clip_{row_i}_{lane_i}_{int(x1)}"
                                svg_lines.append(
                                    f'<clipPath id="{clip_id}">'
                                    f'<rect x="{x1+2:.1f}" y="{lane_y}" width="{bar_w-4:.1f}" height="{LANE_H}"/>'
                                    f'</clipPath>'
                                )
                                svg_lines.append(
                                    f'<text x="{x1 + bar_w/2:.1f}" y="{text_y}" '
                                    f'font-size="10" fill="white" font-weight="bold" '
                                    f'text-anchor="middle" dominant-baseline="middle" '
                                    f'clip-path="url(#{clip_id})">'
                                    f'{html.escape(worker_name)}</text>'
                                )
                        except Exception:
                            pass
                    ...
                    # Separator line
                    svg_lines.append(
                        f'<line x1="0" y1="{y_cursor + row_h - 1}" x2="{svg_w}" '
                        f'y2="{y_cursor + row_h - 1}" stroke="#dde3ed" stroke-width="1"/>'
                    )
                    y_cursor += row_h
                ...
                st.markdown(
                    f'<div style="overflow-x:auto;border:1px solid #d9e2ef;border-radius:14px;padding:10px;">'
                    f'{svg_html}</div>',
                    unsafe_allow_html=True,
                )
                ...
                legend_items = "".join([
                    ...
                    f'<span style="width:14px;height:14px;border-radius:3px;background:{c};display:inline-block;"></span>'
                    f'<span style="font-size:12px;font-weight:800;">{r}</span></span>'
                    for r, c in ROLE_COLORS.items()
                    if r in ["ראש צוות", "דיילת", "מתאם תורים", "מפקח TSA", "שומר TSA", "טרייני ר״צ"]
                ])
                st.markdown(
                    f'<div style="direction:rtl;margin-top:10px;">{legend_items}</div>',
                    unsafe_allow_html=True,
                )
            ...
            if timed.empty:
                ...
                flight_order = (
                    live_flights.copy()
                    if "המראה" in live_flights.columns
                    else live_schedule[["טיסה"]].drop_duplicates()
                )
                flights_sorted = safe_sort_by_time(live_flights, "המראה")["טיסה"].tolist()
                ...
                total_hours = day_max - day_min
                svg_w = LEFT_PAD + total_hours * HOUR_W + RIGHT_PAD
                svg_h = 36 + len(flights_sorted) * ROW_H + 10
                ...
                for h in range(total_hours + 1):
                    ...
                    svg_lines.append(
                        f'<text x="{x+2}" y="20" font-size="11" fill="#6b7fa3">{hour_label}</text>'
                    )

                for row_i, flight_num in enumerate(flights_sorted):
                    y_top = 36 + row_i * ROW_H
                    y_mid = y_top + ROW_H // 2 + 4

                    bg = "#ffffff" if row_i % 2 == 0 else "#f1f5fb"
                    svg_lines.append(
                        f'<rect x="0" y="{y_top}" width="{svg_w}" height="{ROW_H}" fill="{bg}"/>'
                    )
                    ...
                    dest = dest_row.iloc[0]["יעד"] if not dest_row.empty else ""
                    svg_lines.append(
                        f'<text x="{LEFT_PAD - 6}" y="{y_mid - 4}" font-size="11" '
                        f'fill="#071b3a" font-weight="bold" text-anchor="end">{html.escape(short_fnum)}</text>'
                    )
                    svg_lines.append(
                        f'<text x="{LEFT_PAD - 6}" y="{y_mid + 8}" font-size="10" '
                        f'fill="#6b7fa3" text-anchor="end">{html.escape(dest)}</text>'
                    )

                    # Task blocks per flight
                    flight_tasks = timed[timed["טיסה"].astype(str).str.strip() == flight_num.strip()]
                    for _, task in flight_tasks.iterrows():
                        try:
                            ts = pd.to_datetime(task["התחלה"], format="%H:%M")
                            te = pd.to_datetime(task["סיום"],   format="%H:%M")
                            start_h = ts.hour + ts.minute / 60
                            end_h   = te.hour + te.minute / 60
                            if end_h < start_h:
                                end_h += 24

                            x1 = LEFT_PAD + (start_h - day_min) * HOUR_W
                            x2 = LEFT_PAD + (end_h   - day_min) * HOUR_W
                            bar_w = max(x2 - x1, 4)

                            role  = normalize_role_label(task.get("תפקיד בסיס", ""))
                            color = ROLE_COLORS.get(role, "#9fb7d7")
                            worker_name = str(task.get("עובד", ""))
                            short_name = worker_name.split()[-1] if worker_name else ""

                            svg_lines.append(
                                f'<rect x="{x1:.1f}" y="{y_top + 5}" width="{bar_w:.1f}" '
                                f'height="{ROW_H - 10}" rx="5" fill="{color}" opacity="0.88">'
                                f'<title>{html.escape(worker_name)} | {html.escape(role)}</title>'
                                f'</rect>'
                            )
                            if bar_w > 24:
                                svg_lines.append(
                                    f'<text x="{x1 + bar_w/2:.1f}" y="{y_mid}" '
                                    f'font-size="10" fill="white" font-weight="bold" '
                                    f'text-anchor="middle" dominant-baseline="middle">'
