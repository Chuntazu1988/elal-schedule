...
if not daily_file or not employees_file:
    ...
    # ── Landing hero (strip the static CTA pill) ──
    import re as _re
    ...
    _components.html(
        ...
        f"<body style='margin:0;padding:0'>{_landing_no_cta}</body></html>",
    ...
    if not st.session_state.get("show_upload_form"):
        ...
        div[data-testid="stButton"] > button:hover {
            ...
            box-shadow: 0 12px 40px rgba(26,107,255,.6) !important;
        }
        </style>
        """, unsafe_allow_html=True)
        ...
        div[data-testid="stFileUploader"] {
            ...
            border-radius: 16px;
            padding: 16px 20px;
            box-shadow: 0 4px 24px rgba(7,27,58,.12);
            direction: rtl;
        }
        </style>
        """, unsafe_allow_html=True)
        ...
        with col_up:
            st.markdown(
                ...
                'font-weight:900;margin-bottom:12px;">📂 העלאת קבצים</div>',
                unsafe_allow_html=True,
            )
