                ...
                for shift_label, group in shift_groups:
                    ...
                    for _, r in group.iterrows():
                        ...
                        if on_break:
                            ...
                            status = f'☕ בהפסקה מ-<strong>{safe_html(start_display)}</strong>'
                            status_color = "#92400e"
                            status_bg    = "#fef3c7"
                        elif break_done:
                        ...
                        with col_end:
                            if on_break:
                                if st.button("⏹ סיים הפסקה", key=f"brk_end_{btn_key}",
                                             use_container_width=True):
                                    st.session_state["break_log"][name]["end"] = _dt.now().strftime("%H:%M")
                                    st.rerun()
                            else:
...
<script>
(function(){{
  var startTs  = {start_ts};          // Unix seconds (set by server at button click)
  var duration = {break_duration} * 60;  // seconds
