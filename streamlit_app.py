...
def build_shift_map_from_excel(uploaded_file):
    ...
    Returns dict: name_key -> (start, end, original_name)
    Supports formats like "21:00-06:00 ד. שלנ" or standalone "02:00-09:30"
    """
    shift_map = {}
            ...
            for _, cell in raw[col].items():
                ...
                if possible_name:
                    ...
                    if key not in shift_map:
                        shift_map[key] = (current_start, current_end, possible_name)
...
def apply_shift_map_to_employees(employees_df, shift_map_with_names):
    """
    shift_map_with_names: dict of name_key -> (start, end, original_name)
    Match by exact key first, then by 2+ shared words.
    """
    ...
    for idx, row in df.iterrows():
        ...
        if key in shift_map_with_names:
            s, e, _ = shift_map_with_names[key]
            df.at[idx, "תחילת משמרת"] = s
            df.at[idx, "סוף משמרת"]   = e
            continue

        # Strategy 2: fuzzy — 2+ shared words
        emp_words = set(emp_name.split())
        best_match = None
        best_score = 1  # minimum 2 shared words

        for _, (s, e, orig) in shift_map_with_names.items():
            orig_words = set(orig.split())
            shared = len(emp_words & orig_words)
            if shared > best_score:
                best_score = shared
                best_match = (s, e)

        if best_match:
            df.at[idx, "תחילת משמרת"] = best_match[0]
            df.at[idx, "סוף משמרת"]   = best_match[1]
...
def is_within_shift(emp, task_start, task_end):
    ...
    s  = time_to_minutes(shift_start)   # shift start in minutes
    e  = time_to_minutes(shift_end)     # shift end in minutes
    ts = task_start.hour * 60 + task_start.minute
    te = task_end.hour   * 60 + task_end.minute

    # Normalize shift to a continuous range
    if e <= s:
        e += 1440   # overnight shift: e.g. 21:00-06:00 → e=1830

    # Normalize task start into the shift's frame of reference
    # Try task as-is first; if ts is before shift start, try ts+1440
    for ts_norm in [ts, ts + 1440]:
        te_norm = te if te > ts_norm % 1440 else te + 1440
        # Make sure te_norm is after ts_norm
        if te_norm <= ts_norm:
            te_norm += 1440
...
def is_available(assignments, emp_name, start, end):
    """Check if employee has no overlapping task. Handles overnight shifts."""
    ...
    for task in assignments:
        ...
        ee_m = to_minutes(ee)
        if ee_m < es_m:
            ee_m += 1440  # overnight existing task
...
def sort_candidates(candidates, assignments, role, task_start=None, task_end=None):
    ...
    if role == "ראש צוות":
        ...
        return candidates.sort_values(["_shift_priority", "_area_penalty", "_nearby_tasks", "_role_count", "_task_count"])
    ...
    if role == "דייל":
        ...
        return candidates.sort_values(["_shift_priority", "_area_penalty", "_nearby_tasks", "_role_fit", "_task_count"])

    return candidates.sort_values(["_shift_priority", "_area_penalty", "_nearby_tasks", "_task_count"])
...
def build_next_task_labels(result_df, employees_df):
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        # Early morning shift: show break deadline before this task
        # (this label goes on the PREVIOUS task or as prefix)
        ...
        if next_plain:
            ...
            elif shift_type == "night" and not next_plain:
                # Night shift: after first flight → break and return
                next_text = f"הפסקה וחזרה"
            else:
                if is_agent and gap_to_next is not None and gap_to_next >= 30:
                    next_text = f"הפסקה ו{next_plain}"
                else:
            ...
            if needs_break_now:
                ...
                # Night shift: after last flight → break and return
                next_text = f"הפסקה ו{return_text}"
            else:
                next_text = return_text

        # Add break deadline annotation for early morning workers
        deadline_suffix = f" [הפסקה עד {break_deadline}]" if break_deadline and not break_used.get(emp, False) else ""
            ...
            if unassigned_df.empty:
                ...
                for shift_label, group in shift_groups:
                    ...
                    try:
                        ...
                        if 5 <= sh < 12:
                            emoji = "🌅"
                            label = f"משמרת בוקר — {shift_label}"
                        elif 12 <= sh < 18:
                            emoji = "☀️"
                            label = f"משמרת צהריים — {shift_label}"
                        elif 18 <= sh < 22:
                            emoji = "🌆"
                            label = f"משמרת ערב — {shift_label}"
                        else:
                            emoji = "🌙"
                            label = f"משמרת לילה — {shift_label}"
                    except Exception:
                        emoji = "🕐"
                        label = f"משמרת — {shift_label}"
                    ...
                    for _, r in group.iterrows():
                        role_color = "#8e24aa" if r["תפקיד"] == "ראש צוות" else "#5b9bd5"
                        st.markdown(
                            f'<div style="direction:rtl;background:#fff;border:1px solid #e0e8f4;'
                            f'border-right:4px solid {role_color};border-radius:8px;'
                            f'padding:7px 12px;margin-bottom:5px;font-size:13px;">'
                            f'<strong>{safe_html(r["שם"])}</strong>'
                            f'&nbsp;<span style="color:{role_color};font-weight:800;">({safe_html(r["תפקיד"])})</span>'
                            f'</div>',
                            unsafe_allow_html=True,
                        )
            ...
            if not break_rows:
                ...
                for _, r in break_df.iterrows():
                    st.markdown(
                        ...
                        f' ({safe_html(r["תפקיד"])}) | משמרת: {safe_html(r["משמרת"])}'
                        f'<br>'
                        f'חייב/ת לצאת להפסקה עד: <strong style="color:#d97706;">{safe_html(r["הפסקה עד"])}</strong>'
                        f' &nbsp;→&nbsp; טיסה {safe_html(r["טיסה ראשונה"])} | כניסה לשער: {safe_html(r["שעת כניסה לשער"])}'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
