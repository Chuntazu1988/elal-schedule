...
if not daily_file or not employees_file:
    ...
    # ── Global styles for the entire landing page ──
    st.markdown("""
    <style>
    ...
    /* Dark background matching logo palette */
    .stApp, [data-testid="stAppViewContainer"] {
        background: radial-gradient(ellipse 80% 60% at 50% 0%, #0a2540 0%, #04080f 70%) !important;
    }

    /* Let\'s Fly button — teal accent from logo */
    div[data-testid="stButton"] > button {
        background: linear-gradient(120deg, #009e96 0%, #00c9be 100%) !important;
        color: #fff !important;
        ...
        box-shadow: 0 8px 32px rgba(0,201,190,.4) !important;
        letter-spacing: 1px !important;
        text-transform: uppercase !important;
        transition: all .2s ease !important;
    }
    div[data-testid="stButton"] > button:hover {
        box-shadow: 0 14px 40px rgba(0,201,190,.6) !important;
        transform: translateY(-2px) !important;
    }

    /* File uploaders — teal glassmorphism */
    div[data-testid="stFileUploader"] {
        background: rgba(0,201,190,0.06) !important;
        border: 1px solid rgba(0,201,190,0.28) !important;
    ...
    div[data-testid="stFileUploaderDropzone"] {
        background: rgba(0,201,190,0.04) !important;
        border-color: rgba(0,201,190,0.3) !important;
    }
    ...
    # ── Landing hero — strip CTA + fix min-height so iframe doesn't overflow ──
    _style_override = """<style>
    #ischedule-landing {
        min-height: unset !important;
        padding: 28px 24px 24px !important;
    }
    .lp-logo     { font-size: 58px !important; }
    .lp-subtitle { margin: 8px 0 24px !important; }
    .lp-grid     { margin-bottom: 20px !important; gap: 10px !important; }
    .lp-card     { padding: 14px 12px !important; }
    .lp-card-icon { font-size: 22px !important; margin-bottom: 6px !important; }
    </style>"""

    _landing_no_cta = _re.sub(
        r'<div class="lp-cta">.*?</div>', '', LANDING_PAGE, flags=_re.DOTALL
    )
    _components.html(
        f"<!DOCTYPE html><html><head><meta charset='utf-8'></head>"
        f"<body style='margin:0;padding:0;background:transparent'>"
        f"{_style_override}{_landing_no_cta}</body></html>",
    ...
    if not st.session_state.get("show_upload_form"):
        ...
        with col_btn:
            if st.button("✈️  Let's Fly", use_container_width=True):
        ...
        with col_up:
            st.markdown(
                '<div style="text-align:center;color:rgba(255,255,255,.85);'
                'font-size:18px;font-weight:900;margin-bottom:14px;'
                'letter-spacing:.5px;">📂 העלאת קבצים</div>',
                unsafe_allow_html=True,
            )
