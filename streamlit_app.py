...
def is_probable_shift_header(row, name_col):
    ...
    # If the name cell is empty or contains the shift itself, it is a block header.
    if name_value == "":
        return True
    ...
    # Common block title markers seen in your roster.
    if any(marker in name_value for marker in ["ד.", "שלנ", "שלן", "רצ", "בידוק", "חוליה"]):
        return True
...
def normalize_employees(df):
    ...
    # These columns are created from the shift block headers if they are not already present.
