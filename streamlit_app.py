...
def _render_gantt_fullscreen_button(gantt_html, key="gantt_fullscreen"):
    """פותח את אותו HTML של הגאנט בטאב חדש, בלי לשנות את הגאנט עצמו."""
    payload = _json.dumps(gantt_html, ensure_ascii=False)
    _components.html(f"""
    <div style="direction:rtl;text-align:right;padding:0 0 8px 0;">
      <button id="{key}" style="
        background:#0d1f30;
        color:#00c9be;
        border:1px solid rgba(0,201,190,.45);
        border-radius:10px;
        padding:8px 16px;
        font-size:14px;
        font-weight:900;
      ...
      ">⛶ פתח גאנט במסך מלא</button>
    </div>
    <script>
      const html = {payload};
      document.getElementById({ _json.dumps(key) }).onclick = function() {{
...
if _goto_gantt_early and "schedule_df" in st.session_state:
    ...
    _render_gantt_fullscreen_button(_html, key="gantt_fullscreen_late")
