    ...
    try:
        ...
        with tab_gantt:
            ...
            if timed_g.empty:
                ...
                html_gantt = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,user-scalable=no">
<style>
*{{box-sizing:border-box;margin:0;padding:0;}}
body{{font-family:Arial,sans-serif;background:#f8fafc;padding:8px;
...
.task.missing{{
  background:#ffcccc!important;color:#9b0000;cursor:default;
}}
.task.dragging{{
...
#toast{{
  ...
  display:none;z-index:10000;direction:rtl;
  box-shadow:0 4px 16px rgba(0,0,0,0.2);
}}
.toast-ok{{background:#071b3a;color:white;}}
...
const HOUR_PX  = 90;   // pixels per hour — wide enough for names
const LEFT_W   = 90;   // label column width
const TOTAL_W  = LEFT_W + HOURS * HOUR_PX;

// ── lane assignment ─────────────────────────────────────────
function h2px(hStr){{
  const [h,m]=(hStr||"0:0").split(":").map(Number);
  return (h + m/60 - DAY_MIN) * HOUR_PX;
}}
function h2f(hStr){{
  const [h,m]=(hStr||"0:0").split(":").map(Number);
  return h+m/60;
...
inner.style.width = TOTAL_W+"px";
inner.style.position = "relative";

// Hour lines + labels (header padding)
const HDR = 24;
inner.style.paddingTop = HDR+"px";

for(let h=0;h<=HOURS;h++){{
  const x = LEFT_W + h*HOUR_PX;
  const line = document.createElement("div");
  line.className="hour-line";
  line.style.cssText=`left:${{x}}px;top:0;bottom:0;`;
  inner.appendChild(line);

  const lbl = document.createElement("div");
  lbl.className="hour-label";
  lbl.style.cssText=`left:${{x+3}}px;top:4px;`;
  lbl.textContent=String(DAY_MIN+h).padStart(2,"0")+":00";
...
FLIGHTS.forEach((f,fi)=>{{
  ...
  tl.style.cssText=`position:relative;height:${{ROW_H}}px;`;

  assigned.forEach((t,ti)=>{{
    const x1 = h2px(t.start);
    const x2 = h2px(t.end||t.start);
    const bw  = Math.max(x2-x1, 8);
    const top = 7 + t.lane*38;

    const div = document.createElement("div");
    div.className = "task" + (t.missing?" missing":"");
    div.style.cssText = `left:${{x1.toFixed(1)}}px;width:${{bw.toFixed(1)}}px;top:${{top}}px;background:${{t.missing?"#ffcccc":t.color}};`;
    if(t.missing) div.style.color="#9b0000";
    div.textContent = t.worker;
    div.title = `${{t.worker}} | ${{t.role}} | ${{t.start}}–${{t.end}}`;
    div.dataset.fi = fi;
    div.dataset.ti = ti;
...
function addDrag(el, flight, task, fi){{
  function onStart(e){{
    ...
    drag = {{ el, flight, task, fi, startX:src.clientX, startY:src.clientY }};
    el.classList.add("dragging");
    ghost.textContent = task.worker;
    ghost.style.background = task.color;
    ghost.style.display = "flex";
    ghost.style.width = el.offsetWidth+"px";
    moveGhost(src.clientX, src.clientY);
    e.stopPropagation();
...
document.addEventListener("mousemove",e=>{{
  ...
  highlightRow(rowAtPoint(e.clientX,e.clientY));
}});
document.addEventListener("touchmove",e=>{{
  if(!drag)return;
  const t=e.touches[0];
  moveGhost(t.clientX,t.clientY);
  highlightRow(rowAtPoint(t.clientX,t.clientY));
  if(e.cancelable) e.preventDefault();
}},{{passive:false}});

document.addEventListener("mouseup",e=>{{
  if(!drag)return;
  finishDrag(rowAtPoint(e.clientX,e.clientY));
}});
document.addEventListener("touchend",e=>{{
  if(!drag)return;
  const t=e.changedTouches[0];
  finishDrag(rowAtPoint(t.clientX,t.clientY));
}});

function highlightRow(fi){{
  document.querySelectorAll(".frow").forEach(r=>r.classList.remove("drop-target"));
  if(fi>=0&&fi!==drag?.fi){{
    document.querySelector(`.frow[data-fi="${{fi}}"]`)?.classList.add("drop-target");
  }}
}}
...
function finishDrag(targetFi){{
  ...
  if(targetFi<0||targetFi===src.fi)return;

  const tgtFlight=FLIGHTS[targetFi];
  const role=src.task.role;
  const missing=tgtFlight.tasks.find(t=>t.missing&&t.role===role);

  if(!missing&&!tgtFlight.tasks.some(t=>t.role===role)){{
    showToast("❌ אין תפקיד מתאים ("+role+") בטיסה "+tgtFlight.short, false);
    return;
  }}

  const workerName=src.task.worker;

  // Update DOM — src block becomes missing
  src.el.textContent="❌ חסר "+role;
  src.el.style.background="#ffcccc";
  src.el.style.color="#9b0000";
  src.el.classList.add("missing");
  src.task.missing=true;
  src.task.worker="❌ חסר "+role;

  // Update target missing block
  if(missing){{
    missing.worker=workerName;
    missing.missing=false;
    // find the DOM element for this task
    const tgtEl=document.querySelector(`.task[data-idx="${{missing.idx}}"]`);
    if(tgtEl){{
      tgtEl.textContent=workerName;
      tgtEl.style.background=missing.color||src.task.color;
      tgtEl.style.color="white";
      tgtEl.classList.remove("missing");
      addDrag(tgtEl, tgtFlight, missing, targetFi);
    }}
  }}

  showToast("✅ "+workerName+" → "+tgtFlight.short+" ("+role+")", true);

  // Sync to Streamlit session_state via postMessage
  ...
  try{{window.parent.postMessage({{isStreamlitMessage:true,type:"streamlit:setComponentValue",args:{{value:payload}}}},"*");}}catch(_){{}}
  try{{if(window.Streamlit)window.Streamlit.setComponentValue(payload);}}catch(_){{}}
}}
...
[["ראש צוות","#8e24aa"],["דיילת","#5b9bd5"],["מתאם תורים","#f0a000"],
 ["מפקח TSA","#d32f2f"],["שומר TSA","#2e7d32"],["טרייני ר״צ","#f9a825"]].forEach(([r,c])=>{{
  ...
  lDiv.appendChild(d);
                ...
                _components.html(
                    html_gantt,
                    height=max(500, len(flights_g) * 55 + 140),
                    scrolling=False,
                )
