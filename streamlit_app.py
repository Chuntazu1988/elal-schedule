...
def normalized_interval(start_dt, end_dt):
    s = dt_minutes(start_dt)
    e = dt_minutes(end_dt)
    if e <= s:
        e += 24 * 60
    return s, e
...
def intervals_overlap(start1, end1, start2, end2, buffer_minutes=5):
    s1, e1 = normalized_interval(start1, end1)
    s2, e2 = normalized_interval(start2, end2)

    for shift in (0, 24 * 60, -24 * 60):
        ss2 = s2 + shift
        ee2 = e2 + shift
        if not (e1 + buffer_minutes <= ss2 or s1 >= ee2 + buffer_minutes):
            return True
    return False
...
def normalize_role_label(role):
    ...
    if role.startswith("ראש צוות") or role in {"רצ", "ר״צ", 'ר"צ'}:
        return "ראש צוות"
    ...
    if role.startswith("שומר"):
        return "שומר TSA"
    if role.startswith("טרייני") or "טרייני ר" in role or "טרייני רצ" in role:
        return "טרייני ר״צ"
    return role


def short_role_label(role):
...
def role_area(role):
    ...
    if role in {"ראש צוות", "דיילת", "מתאם תורים", "מפקח TSA", "שומר TSA", "טרייני ר״צ"}:
        return "שערי יציאה"
    return "דלפקים"
...
def is_available(assignments, emp_name, start, end):
    """Strict no-overlap rule. TSA time window starts 120 minutes before ETD."""
    for task in assignments:
        if task["עובד"] != emp_name:
            continue
        if clean_text(task.get("התחלה", "")) == "" or clean_text(task.get("סיום", "")) == "":
        ...
        if intervals_overlap(start, end, existing_start, existing_end, buffer_minutes=5):
            return False
...
def area_switch_penalty(assignments, emp, role):
    ...
    The worker is never locked to one role for the whole shift.
    This only helps avoid unnecessary movement between check-in counters and gates.
    """
    emp_name = emp["שם"]
...
def flight_has_mentor_teamlead(assignments_for_flight, employees_df, training_type):
    ...
    for task in assignments_for_flight:
        ...
        for col in required_cols:
            if str(emp.iloc[0].get(col, "")).strip() == "כן":
                return True
    return False
...
def employee_shift_text(employees_df, emp_name):
    ...
    start = clean_text(emp.iloc[0].get("תחילת משמרת", ""))
    end = clean_text(emp.iloc[0].get("סוף משמרת", ""))
...
def next_task_plain_text(df, current_idx, employee_name):
    current_row = df.loc[current_idx]

    future = df[
        (df["עובד"] == employee_name)
    ...
    future["_start_dt"] = pd.to_datetime(
        future["התחלה"],
        format="%H:%M",
        errors="coerce"
    )
    ...
    future = future[future["_start_dt"] >= current_end]

    if future.empty:
        return None

    next_row = future.sort_values("_start_dt").iloc[0]

    role = short_role_label(next_row["תפקיד"])
    flight = str(next_row["טיסה"]).strip()

    return f"{flight} {role}"


def build_next_task_labels(result_df, employees_df):
    """
    Final worker text format:
    נטע ונטוררו - הפסקה ו315 ראש צוות (02:00-11:00)
    נטע ונטוררו - רענון ו7 ר״צ (14:00-01:30)
    נטע ונטוררו - הפסקה וחזרה (02:00-09:30)
    נטע ונטוררו - 315 ראש צוות (02:00-11:00)
    """
    df = result_df.copy()
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        if emp_match.empty:
            next_plain = next_task_plain_text(timed_df, idx, emp) or "חזרה"
            df.loc[idx, "טקסט עובד"] = f"{emp} - {next_plain}"
            df.loc[idx, "המשך אזורי"] = next_plain
            continue

        emp_row = emp_match.iloc[0]
        shift = employee_shift_text(employees_df, emp)
        shift_suffix = f" ({shift})" if shift else ""
        ...
        if next_plain:
            if needs_break_now:
                break_used[emp] = True
                if gap_to_next is not None and gap_to_next >= break_minutes + HELP_THRESHOLD_MINUTES:
...
def build_output_table(flights_df, result_labeled, employees_df):
    ...
    for _, flight in flights_df.iterrows():
        ...
        for _, task in tasks.iterrows():
            ...
            if "ראש צוות" in role or "טרייני" in role or "מתאם" in role or "מפקח TSA" in role:
                management_lines.append(item)
                if base_role not in left_roles:
                    left_roles.append(base_role)
            elif "דייל" in role or "שומר TSA" in role or "בדיקת טרייני" in role:
                agents_lines.append(item)
                if base_role not in right_roles:
                    right_roles.append(base_role)
        ...
        rows.append({
            ...
            "כותרת שמאל": " / ".join(left_roles) if left_roles else "ניהול",
            "כותרת ימין": " / ".join(right_roles) if right_roles else "דיילים",
...
def render_flight_card(row):
    ...
    with c1:
        left_title = str(row.get("כותרת שמאל", "ניהול"))
        st.markdown(f'<div class="panel-title">👔 {safe_html(left_title)}</div>', unsafe_allow_html=True)
    ...
    with c2:
        right_title = str(row.get("כותרת ימין", "דיילים"))
        st.markdown(f'<div class="panel-title">🧍 {safe_html(right_title)}</div>', unsafe_allow_html=True)
