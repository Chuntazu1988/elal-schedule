...
def extract_shift_range_from_text(value):
    """
    Extract shift range from text such as:
    ד. שלנ 02:00-09:30
    רצ 12:30 - 21:00
    בידוק חוליה 23:30-08:30
    02:00:00-09:30:00
    """
    text = clean_text(value)

    # Normalize common dash symbols and invisible spaces.
    text = (
        text.replace("–", "-")
            .replace("—", "-")
            .replace("−", "-")
    ...
    # Find two time-like values anywhere in the same cell.
...
def extract_shift_range_from_row(row):
    """Search all cells in a row and also combined row text."""
    values = [clean_text(v) for v in row.values]
    combined = " | ".join(values)

    start, end = extract_shift_range_from_text(combined)
    if start and end:
        return start, end
...
def is_probable_shift_header(row, name_col):
    ...
    if extract_shift_range_from_text(name_value)[0]:
        return True

    if any(marker in name_value for marker in ["ד.", "שלנ", "שלן", "רצ", "ר״צ", 'ר"צ', "בידוק", "חוליה"]):
        return True
...
def normalize_employees(df):
    ...
    # Sometimes the first shift is hidden in a column title because Excel/Pandas used it as header.
    for col_name in df.columns:
        found_start, found_end = extract_shift_range_from_text(col_name)
    ...
    for idx, row in df.iterrows():
        ...
        if found_start and found_end:
            ...
            if is_probable_shift_header(row, "שם"):
    ...
    # Remove block title rows that may have survived.
