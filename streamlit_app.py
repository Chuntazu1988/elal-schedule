...
def render_flight_card_with_swap(row, schedule_df, employees_df):
    ...
    def render_lines_with_swap(panel_lines):
        for line in panel_lines:
            ...
            popup_key = f"popup_open_{task_idx}"
            ...
            with col_line:
                st.markdown(
                    f'<div class="assignment-line {style_cls}" style="margin-bottom:0">{safe_html(text_part)}</div>',
                    unsafe_allow_html=True,
                )
            with col_arrow:
                arrow = "▲" if st.session_state[popup_key] else "▼"
                if st.button(arrow, key=f"arrow_{task_idx}", help="החלף עובד"):
            ...
            if st.session_state[popup_key]:
                ...
                if not candidates:
                    ...
                    with bc1:
                        if st.button("✅ אשר החלפה", key=f"swap_confirm_{task_idx}", use_container_width=True):
                            ...
                            st.rerun()
                    with bc2:
                        if st.button("✖ ביטול", key=f"swap_cancel_{task_idx}", use_container_width=True):
                            st.session_state[popup_key] = False
                            st.rerun()
