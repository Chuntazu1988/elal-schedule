...
if not daily_file or not employees_file:
    ...
    .upload-btn button {
        ...
        box-shadow: 0 4px 20px rgba(26,107,191,0.5) !important;
        transition: transform 0.15s !important;
    }
    .upload-btn button:hover { transform: scale(1.05) !important; }
    </style>
    """, unsafe_allow_html=True)

    # Full-screen black layout with logo
    st.markdown(f'''
    <div style="
    ...
    ">
        <img src="data:image/jpeg;base64,{_logo_b64}"
             style="max-width: 75vw; max-height: 72vh; object-fit: contain;"
             alt="iSchedule" />
    </div>
    ''', unsafe_allow_html=True)
    ...
    with col2:
        if st.button("📁  הוסף קבצים", use_container_width=True, key="open_upload"):
            st.session_state["show_upload"] = True

    if st.session_state.get("show_upload"):
        with col2:
            st.markdown("<div style='direction:rtl;color:white;font-size:14px;text-align:center;margin-top:16px;'>", unsafe_allow_html=True)
            daily_file   = st.file_uploader("📅 קובץ סידור יומי (Excel)", type=["xlsx"], key="landing_daily")
            employees_file = st.file_uploader("👥 קובץ עובדים / הסמכות (Excel)", type=["xlsx"], key="landing_emp")
            st.markdown("</div>", unsafe_allow_html=True)
