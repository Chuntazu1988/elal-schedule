...
_DAY_HE = {
    ...
    "Saturday": "שבת", "Sunday": "יום ראשון",
}

def get_israel_now() -> datetime:
    """מחזיר datetime נוכחי בשעון ישראל (IST/IDT אוטומטי)."""
    return datetime.now(tz=_IL_TZ)
    ...
    for _file_idx, fobj in enumerate(fids_file_objects):
        ...
        try:
            ...
            if fc:
                ...
                # קובץ FIDS שני (מחר) — כל הטיסות שממריאות לפני 02:00
                if _file_idx == 1:
                    _dep_src = _find_src(fids_df.columns, ["std", "scheduled", "departure", "המראה", "etd", "time"])
                    if _dep_src:
                        def _before_0200(v):
                        ...
                        fids_df = fids_df[fids_df[_dep_src].apply(_before_0200)]
...
_now_disp = get_israel_now()
_day_he   = _DAY_HE.get(_now_disp.strftime("%A"), _now_disp.strftime("%A"))
st.markdown(
    ...
    f'📅 {_day_he}, {_now_disp.strftime("%d/%m/%Y")}'
    f'&nbsp;&nbsp;|&nbsp;&nbsp;'
    f'🕐 שעה נוכחית (ישראל): <strong style="font-size:15px;">{_now_disp.strftime("%H:%M")}</strong>'
    f'</div>',
    unsafe_allow_html=True,
)
...
if st.button("🚀 בנה שיבוץ", use_container_width=True):
    try:
        now_il      = get_israel_now()
        schedule_df = build_schedule(flights_editor_df, employees_df, current_time=now_il)
        ...
        st.session_state["employees_snap"]    = employees_df.copy()
        st.session_state["schedule_built_at"] = now_il
        st.success(f"השיבוץ נבנה בהצלחה — {now_il.strftime('%H:%M')} ✅")
    except Exception as exc:
...
if "schedule_df" in st.session_state:
    try:
        ...
        # ── חותמת זמן יצירת השיבוץ ────────────────────────────────────────────
        if "schedule_built_at" in st.session_state:
            _built   = st.session_state["schedule_built_at"]
            _day_b   = _DAY_HE.get(_built.strftime("%A"), _built.strftime("%A"))
            st.markdown(
                f'<div style="direction:rtl;text-align:right;font-size:11.5px;'
                f'color:#6b7a8d;margin-bottom:4px;">'
                f'⏱ שיבוץ נוצר: {_day_b} {_built.strftime("%d/%m/%Y")} בשעה '
                f'<strong>{_built.strftime("%H:%M")}</strong></div>',
                unsafe_allow_html=True,
            )
