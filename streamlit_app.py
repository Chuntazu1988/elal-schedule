...
def name_key(value):
    """Normalize employee names so the same person cannot be assigned twice on one flight."""
...
def normalize_employees(df):
    ...
    df = df[df["שם"] != ""].copy()
    df["_שם_KEY"] = df["שם"].apply(name_key)
...
def build_schedule(flights_df, employees_df):
    ...
    flights_df["_טיסה_KEY"] = flights_df["טיסה"].astype(str).str.replace(" ", "", regex=False).str.upper()
    flights_df = flights_df.drop_duplicates(subset=["_טיסה_KEY"], keep="first").copy()
        ...
        for role in ROLE_ORDER:
            ...
            for i in range(amount):
                ...
                candidates = employees_df[
                    ...
                    (~employees_df["_שם_KEY"].isin(used_on_flight))
                ].copy()
...
def render_flight_card(row):
    ...
    # Keep the header clean and avoid nested HTML chips that can render as broken text on mobile.
    required_line = " | ".join([part.strip() for part in reqs.split("|") if part.strip()])
    if not required_line:
        required_line = "לא הוגדרו תפקידים"

    st.markdown(
        f"""
        <div dir="rtl" style="text-align:right;border:1px solid #d9e2ef;border-radius:16px;
             margin-bottom:10px;background:white;box-shadow:0 2px 10px rgba(0,0,0,.06);overflow:hidden;">
            <div style="background:#071b3a;color:white;padding:12px 16px;">
                ...
                <div style="margin-top:10px;background:#123b70;border:1px solid #315c91;
                    border-radius:10px;padding:8px 10px;color:white;font-size:13px;font-weight:800;">
                    תפקידים דרושים: {safe_html(required_line)}
                </div>
            </div>
        </div>
        """,
...
# Remove duplicate flight rows from the original daily report before editing.
flights_df["_טיסה_KEY"] = flights_df["טיסה"].astype(str).str.replace(" ", "", regex=False).str.upper()
flights_df = flights_df.drop_duplicates(subset=["_טיסה_KEY"], keep="first").drop(columns=["_טיסה_KEY"])
