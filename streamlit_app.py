...
with st.expander("📡 טעינת נתוני FIDS (גייט / רישוי / נוסעים)", expanded=False):
    st.caption("העלה קובץ Excel שיוצא מה-FIDS עם עמודות: טיסה, גייט, רישוי, נוסעים")
    fids_file = st.file_uploader("קובץ FIDS", type=["xlsx","csv"], key="fids_uploader")
    if fids_file:
        try:
            ...
            # Find flight column
            fc = next((c for c in fids_df.columns if "טיסה" in c or c.lower() in {"flight","flightno","flight no"}), None)
            if not fc:
                st.error("לא נמצאה עמודת טיסה בקובץ. ודא שיש עמודה בשם 'טיסה' או 'Flight'.")
            else:
                ...
                COL_MAP = {
                    ...
                    "רישוי":["רישוי","reg","Reg","registration","Registration","REG"],
                    "נוסעים":["נוסעים","pax","Pax","PAX","passengers","Passengers"],
                }
                filled = 0
