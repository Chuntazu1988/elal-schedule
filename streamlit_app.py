    ...
    for _, flight in flights_df.iterrows():
        ...
        for _, task in tasks.iterrows():
            ...
            if "❌" not in worker and worker and " - " in text_value:
                ...
                text_value = f"{name_part} ({base_role}) - {rest}"
            elif "❌" not in worker and worker:
                text_value = f"{worker} ({base_role})"
...
[["ראש צוות","#8e24aa"],["דיילת","#5b9bd5"],["מתאם תורים","#f0a000"],["מפקח TSA","#d32f2f"],["שומר TSA","#2e7d32"],["טרייני ר״צ","#f9a825"]].forEach(([r,c])=>{{
  ...
  d.innerHTML=`<span class="ld" style="background:${{c}}"></span><span>${{r}}</span>`;lDiv.appendChild(d);
                ...
                # Show button that opens gantt in new tab
                import base64 as _b64
                encoded = _b64.b64encode(gantt_page.encode("utf-8")).decode()
                open_btn_html = f"""
<a href="data:text/html;base64,{encoded}"
   target="_blank"
   style="display:inline-block;background:#071b3a;color:white;padding:12px 28px;
          border-radius:12px;font-size:15px;font-weight:900;text-decoration:none;
          box-shadow:0 4px 14px rgba(7,27,58,0.25);">
  📅 פתח גאנט בחלון נפרד
</a>"""
                st.markdown(open_btn_html, unsafe_allow_html=True)
                st.caption("הגאנט ייפתח בטאב חדש בדפדפן — ניתן לגרור עובדים בין טיסות.")
