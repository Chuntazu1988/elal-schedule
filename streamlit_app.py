...
def build_next_task_labels(result_df, employees_df):
    """
    Display rules:
    1. Color is based only on the current role.
    2. 'עזרה' is used only when the gap is very large.
       Otherwise the worker is sent directly to the next flight.
    3. Break/refresher must be reflected whenever the worker is entitled to one.
    4. Shift hours are appended to every worker line.
    """
    df = result_df.copy()
    ...
    # Thresholds:
    # Under this, go straight to next flight.
    # Above this, if already took break and still has a large gap, show "עזרה".
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        if next_plain:
            if needs_break_now:
                ...
                # If there is time for break before next flight, show it.
                # Use "עזרה" only if gap is very large after the break.
                if gap_to_next is not None and gap_to_next >= break_minutes:
                    ...
                    # Not enough gap now, still reflect planned break before continuation.
                    next_text = f"{break_text} ו{next_plain}"
            else:
                # No break needed/left. Prefer another flight directly.
                # Only mention help if the gap is very large.
