...
if "schedule_df" in st.session_state:
    try:
        ...
        with tab_gantt:
            ...
            st.caption("כל שורה = טיסה, צבע = תפקיד, טקסט = שם עובד. גלול ימינה לשעות מאוחרות יותר.")
            ...
            if timed_g.empty:
                ...
                total_hours = g_day_max - g_day_min

                flights_g = safe_sort_by_time(live_flights, "המראה")["טיסה"].tolist()

                # Search filter
                search_g = st.text_input("🔎 חיפוש:", key="gantt_search")
                if search_g:
                    mask = timed_g.astype(str).apply(
                        lambda r: r.str.contains(search_g, case=False, na=False).any(), axis=1
                    )
                    matched = timed_g[mask]["טיסה"].unique().tolist()
                    flights_g = [f for f in flights_g if f in matched]

                # Legend
                legend_html = '<div style="display:flex;flex-wrap:wrap;gap:8px;direction:rtl;margin-bottom:10px;">'
                for role, color in [("ראש צוות","#8e24aa"),("דיילת","#5b9bd5"),("מתאם תורים","#f0a000"),
                                     ("מפקח TSA","#d32f2f"),("שומר TSA","#2e7d32"),("טרייני ר״צ","#f9a825")]:
                    legend_html += (
                        f'<span style="display:flex;align-items:center;gap:4px;font-size:12px;font-weight:800;">'
                        f'<span style="width:14px;height:14px;border-radius:3px;background:{color};display:inline-block;"></span>'
                        f'{safe_html(role)}</span>'
                    )
                legend_html += '</div>'
                st.markdown(legend_html, unsafe_allow_html=True)

                # Build SVG — fixed HOUR_W=80, scroll horizontally
                HOUR_W   = 80
                LANE_H   = 32
                LANE_PAD = 4
                LEFT_PAD = 100
                HDR_H    = 30
                ROW_PAD  = 5
                SVG_W    = LEFT_PAD + total_hours * HOUR_W + 20

                def assign_lanes_g(tasks):
                    lanes, result = [], []
                    for _, t in tasks.iterrows():
                        try:
                            sh = pd.to_datetime(t["התחלה"], format="%H:%M")
                            eh = pd.to_datetime(t["סיום"],   format="%H:%M")
                            s = sh.hour + sh.minute/60
                            e = eh.hour + eh.minute/60
                            if e < s: e += 24
                        except Exception:
                            result.append((0, 0, 0, t))
                            continue
                        placed = -1
                        for li, lane in enumerate(lanes):
                            if all(s >= le or e <= ls for ls, le in lane):
                                lane.append((s, e)); placed = li; break
                        if placed < 0:
                            lanes.append([(s, e)]); placed = len(lanes) - 1
                        result.append((placed, s, e, t))
                    return result, len(lanes) if lanes else 1

                # Pre-calculate row heights
                flight_meta = []
                for fnum in flights_g:
                    ftasks = timed_g[timed_g["טיסה"].astype(str).str.strip() == fnum.strip()]
                    assigned, nlanes = assign_lanes_g(ftasks)
                    row_h = nlanes * (LANE_H + LANE_PAD) + ROW_PAD * 2
                    dest_r = live_flights[live_flights["טיסה"] == fnum]
                    dest_v = dest_r.iloc[0]["יעד"] if not dest_r.empty else ""
                    flight_meta.append({"fnum": fnum, "short": fnum.replace("LY","").strip(),
                                        "dest": dest_v, "assigned": assigned, "row_h": row_h})

                SVG_H = HDR_H + sum(m["row_h"] for m in flight_meta) + 8
                ...
                for h in range(total_hours + 1):
                    x = LEFT_PAD + h * HOUR_W
                    lines.append(f'<line x1="{x}" y1="{HDR_H-4}" x2="{x}" y2="{SVG_H}" stroke="#dde3ed" stroke-width="1"/>')
                    lines.append(f'<text x="{x+3}" y="{HDR_H-8}" font-size="11" fill="#8a9ab5">{(g_day_min+h):02d}:00</text>')

                # Rows
                y = HDR_H
                for ri, m in enumerate(flight_meta):
                    bg = "#ffffff" if ri % 2 == 0 else "#f1f5fb"
                    lines.append(f'<rect x="0" y="{y}" width="{SVG_W}" height="{m["row_h"]}" fill="{bg}"/>')

                    mid_y = y + m["row_h"] // 2
                    lines.append(f'<text x="{LEFT_PAD-6}" y="{mid_y-4}" font-size="12" fill="#071b3a" font-weight="bold" text-anchor="end">{safe_html(m["short"])}</text>')
                    lines.append(f'<text x="{LEFT_PAD-6}" y="{mid_y+10}" font-size="10" fill="#8a9ab5" text-anchor="end">{safe_html(m["dest"])}</text>')

                    for (li, sh, eh, task) in m["assigned"]:
                        x1 = LEFT_PAD + (sh - g_day_min) * HOUR_W
                        bw = max((eh - sh) * HOUR_W, 6)
                        ly = y + ROW_PAD + li * (LANE_H + LANE_PAD)
                        ty = ly + LANE_H // 2 + 4
                        worker = str(task.get("עובד", ""))
                        role   = normalize_role_label(str(task.get("תפקיד בסיס", "")))
                        color  = ROLE_COLORS_G.get(role, "#9fb7d7") if "❌" not in worker else "#ffcccc"
                        txt_c  = "white" if "❌" not in worker else "#9b0000"

                        clip_id = f"cl{ri}_{li}_{int(x1)}"
                        lines.append(f'<clipPath id="{clip_id}"><rect x="{x1+3:.0f}" y="{ly}" width="{max(bw-6,0):.0f}" height="{LANE_H}"/></clipPath>')
                        lines.append(f'<rect x="{x1:.1f}" y="{ly}" width="{bw:.1f}" height="{LANE_H}" rx="6" fill="{color}" opacity="0.9"><title>{safe_html(worker)} | {safe_html(role)}</title></rect>')
                        if bw > 20:
                            lines.append(f'<text x="{x1+bw/2:.1f}" y="{ty}" font-size="10" fill="{txt_c}" font-weight="bold" text-anchor="middle" clip-path="url(#{clip_id})">{safe_html(worker)}</text>')

                    lines.append(f'<line x1="0" y1="{y+m["row_h"]-1}" x2="{SVG_W}" y2="{y+m["row_h"]-1}" stroke="#dde3ed" stroke-width="1"/>')
                    y += m["row_h"]
                ...
                st.markdown(
                    ...
                    f'border:1px solid #d9e2ef;border-radius:14px;padding:8px;">'
                    f'{svg_out}</div>',
                    unsafe_allow_html=True,
                )
