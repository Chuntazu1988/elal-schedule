...
def build_next_task_labels(result_df, employees_df):
    """
    Display rules:
    - Color is based only on the current role.
    - Text after dash reflects operational continuation:
      הפסקה ו2365 רצ
      הפסקה עזרה ו2365 רצ
      הפסקה וחזרה לדלפקים / הפסקה וחזרה הביתה
      חזרה לדלפקים / חזרה הביתה
    """
    df = result_df.copy()
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        needs_break_now = break_text and not break_used.get(emp, False)

        if next_plain:
            # If there is a big enough gap, put break and assistance before next flight.
            if needs_break_now:
                break_used[emp] = True
                if gap_to_next is not None and gap_to_next >= break_minutes + 25:
                    next_text = f"{break_text} עזרה ו{next_plain}"
                else:
                    next_text = f"{break_text} ו{next_plain}"
            else:
                # No break needed/left. If the gap is large, send to help/counters before next task.
                if gap_to_next is not None and gap_to_next >= 60:
