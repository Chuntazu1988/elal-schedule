            ...
            for row_i, cell in raw[col].items():
                ...
                if m:
                    ...
                    if is_blocked_label(cell_text):
                        ...
                        # Also update shift times from blocked header
                        # (worker IS on shift during these hours, just doing a different role)
                        current_start = s
                        current_end   = e
                    else:
                        current_start = s
                ...
                # Check for special inline format: "NAME-תגבור בין HH:MM-HH:MM ואז בין HH:MM-HH:MM"
                # This means the employee is available in specific windows only
                if 'תגבור' in cell_text or 'ואז בין' in cell_text:
                    # Extract name part (before the dash)
                    name_part = cell_text.split('-')[0].strip() if '-' in cell_text else cell_text
                    pn = clean_roster_name(name_part)
                    if pn and current_start and current_end:
                        key = name_key(pn)
                        # Extract all time ranges in this cell — these are availability windows
                        ...
                        if key not in shift_map:
                            shift_map[key] = {
                                ...
                                "available_windows": [(normalize_time_text(ws), normalize_time_text(we)) for ws, we in avail_windows],
                            }
                        else:
                            if avail_windows:
                                shift_map[key]["available_windows"] = [(normalize_time_text(ws), normalize_time_text(we)) for ws, we in avail_windows]
                    continue
...
def build_next_task_labels(result_df, employees_df):
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        # Night shift: check if break window (00:00-02:00) is approaching or passed
        night_needs_break = False
        night_break_window_note = ""
        if shift_type == "night" and not break_used.get(emp, False):
            # Normalize task_end_m to shift frame (shift starts ~21:00)
            te_norm = task_end_m if task_end_m < 21 * 60 else task_end_m - 1440
            # If task ends before 02:00 and break window hasn't been used
            if te_norm <= NIGHT_BREAK_WINDOW_END:
                night_needs_break = True
                night_break_window_note = " (הפסקה בין 00:00-02:00)"

        if next_plain:
            if needs_break_now:
                break_used[emp] = True
                next_text = f"{break_text} ו{next_plain}"
            elif night_needs_break:
                break_used[emp] = True
