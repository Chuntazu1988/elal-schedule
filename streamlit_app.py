            ...
            if unassigned_df.empty:
                ...
                for shift_label, group in shift_groups:
                    ...
                    for _, r in group.iterrows():
                        ...
                        # Build status text
                        if on_break:
                            status = f'☕ בהפסקה מ-<strong>{safe_html(break_info["start"])}</strong>'
                            status_color = "#92400e"
                            status_bg    = "#fef3c7"
                        elif break_done:
                            status = f'✅ הפסקה: {safe_html(break_info["start"])} – {safe_html(break_info["end"])}'
                            status_color = "#065f46"
                            status_bg    = "#d1fae5"
                        else:
                            status = ""
                        ...
                        with col_start:
                            if not on_break and not break_done:
                                if st.button("▶ התחל הפסקה", key=f"brk_start_{btn_key}",
                                    ...
                                    st.session_state["break_log"][name] = {"start": now, "end": None}
                                    st.rerun()
                            elif on_break:
                        ...
                        with col_end:
                            if on_break:
                                if st.button("⏹ סיים הפסקה", key=f"brk_end_{btn_key}",
                                             use_container_width=True):
                                    now = _dt.now().strftime("%H:%M")
                                    st.session_state["break_log"][name]["end"] = now
                                    st.rerun()
                            else:
...
(function(){{
  ...
  function update() {{
    var elapsed = Math.floor((new Date() - startToday) / 1000);
    ...
    if (remaining <= 0) {{
      ...
      var s = remaining % 60;
      el.textContent = "נותרו " + m + ":" + String(s).padStart(2,"0") + " דקות מתוך {break_duration} דק׳";
    }}
  }}
  update();
