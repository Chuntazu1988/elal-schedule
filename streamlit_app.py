...
with st.sidebar:
    ...
    _sidebar_daily = st.file_uploader("קובץ סידור יומי", type=["xlsx"], key="sidebar_daily")
    _sidebar_emp   = st.file_uploader("קובץ עובדים / הסמכות", type=["xlsx"], key="sidebar_emp")

    # If uploaded via sidebar — store bytes
    if _sidebar_daily:
        st.session_state["daily_bytes"] = _sidebar_daily.read()
        st.session_state["daily_name"]  = _sidebar_daily.name
        st.session_state["files_ready"] = True
    if _sidebar_emp:
        st.session_state["emp_bytes"] = _sidebar_emp.read()
        st.session_state["emp_name"]  = _sidebar_emp.name
        st.session_state["files_ready"] = True

# Build file objects from bytes stored in session_state
def _make_bytesio(key_bytes, key_name):
    b = st.session_state.get(key_bytes)
    n = st.session_state.get(key_name, "file.xlsx")
    if b:
        f = _io.BytesIO(b)
        f.name = n
        return f
...
daily_file     = _make_bytesio("daily_bytes", "daily_name")
employees_file = _make_bytesio("emp_bytes",   "emp_name")
...
if not st.session_state.get("files_ready", False):
    ...
    # ── עמוד נחיתה פשוט — ללא CSS overlay ──────────────────────────────────
    st.markdown("""
    <style>
    html, body, .stApp { background-color: #0a0a1a !important; color: #dde6f0 !important; }
    header, footer, #MainMenu, section[data-testid="stSidebar"] { display: none !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    <div style="text-align:center;padding-top:40px;padding-bottom:20px">
        <img src="data:image/jpeg;base64,{_logo_b64}"
             style="max-width:80vw;max-height:50vh;object-fit:contain;" alt="iSchedule"/>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(
            ...
            "✈️ iSchedule — העלה קבצים להתחלה</div>",
            unsafe_allow_html=True,
        )
        uf_daily = st.file_uploader("📅 סידור יומי (Excel)",  type=["xlsx"], key="lp_daily")
        uf_emp   = st.file_uploader("👥 קובץ הסמכות (Excel)", type=["xlsx"], key="lp_emp")
        if uf_daily and uf_emp:
            st.session_state["daily_bytes"] = uf_daily.read()
            st.session_state["daily_name"]  = uf_daily.name
            st.session_state["emp_bytes"]   = uf_emp.read()
            st.session_state["emp_name"]    = uf_emp.name
            st.session_state["files_ready"] = True
            st.rerun()
        elif uf_daily:
            st.success("✅ סידור יומי נטען — כעת העלה את קובץ ההסמכות")
        elif uf_emp:
            st.success("✅ קובץ הסמכות נטען — כעת העלה את הסידור היומי")
...
try:
    ...
    st.error("לא הצלחתי לקרוא את הקבצים. נסה להעלות מחדש.")
    st.exception(exc)
    st.stop()
