...
def apply_shift_map_to_employees(employees_df, shift_map):
    """
    Match employees to shift times using two strategies:
    1. Exact key match (name_key)
    2. Fuzzy match: at least 2 words in common between employee name and shift name
    """
    ...
    # Build reverse lookup: set of words -> (start, end, original_name)
    # shift_map keys are name_key strings, values are (start, end)
    # We also need the original names for fuzzy matching
    shift_words = {}  # frozenset of words -> (start, end)
    for k, (s, e) in shift_map.items():
        # reconstruct words from key is hard — store separately
        pass
    ...
    for idx, row in df.iterrows():
        ...
        # Strategy 2: fuzzy — already found in shift_map via fuzzy_map below
        # (handled after loop)
...
def is_within_shift(emp, task_start, task_end):
    ...
    # If no shift hours — employee is not working today, cannot be assigned
    if not is_time_text(shift_start) or not is_time_text(shift_end):
        return False
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        if next_plain:
            if needs_break_now:
                ...
                # For agents with gap >= 30 min — send to break then next task
            ...
            if needs_break_now:
                ...
                # For agents with gap >= 30 min after last task — send back to counters
                if is_agent and gap_to_next is None:
...
if "schedule_df" in st.session_state:
    try:
        ...
        tab_schedule, tab_gantt, tab_missing, tab_available, tab_unassigned, tab_workload, tab_continuity, tab_raw = st.tabs(
            ["✈️ לוח מבצעים", "📅 גאנט", "❌ חוסרים", "🟡 פנויים באולם", "🏠 לא משובצים", "📊 עומס עובדים", "🧭 רצף אזורי", "🧾 פירוט גולמי"]
        )
