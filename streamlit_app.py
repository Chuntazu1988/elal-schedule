...
body{{background:#f8fafc;padding:6px;overscroll-behavior:none;}}
#legend{{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:8px;direction:rtl;}}
.li{{display:flex;align-items:center;gap:4px;font-size:12px;font-weight:800;color:#071b3a;}}
...
#info{{direction:rtl;font-size:13px;font-weight:800;padding:8px 12px;
       border-radius:8px;margin-top:8px;display:none;}}
.ok{{background:#d4edda;color:#155724;}}
.err{{background:#ffe8e8;color:#9b0000;}}
#hint{{direction:rtl;font-size:11px;color:#6b7fa3;margin-bottom:6px;}}
</style>
</head>
<body>
<div id="legend"></div>
<p id="hint">💡 גרור משבצת עובד לשורת טיסה אחרת. גלול ימינה/שמאלה לראות את כל השעות.</p>
<div id="scroll-wrap"><canvas id="c"></canvas></div>
<div id="info"></div>
...
const HDR_H    = 32;
const ROW_PAD  = 6;
const HOUR_W   = 90;   // fixed wide — scroll to see all
...
function buildLayout(){{
  const W=LEFT+HOURS*HOUR_W+24;
  canvas.width=W;
  let y=HDR_H;
  layout=flights.map(f=>{{
    const assigned=assignLanes(f.tasks);
    const nL=assigned.length?Math.max(...assigned.map(t=>t.lane))+1:1;
    const rowH=nL*(LANE_H+LANE_PAD)+ROW_PAD*2;
    const row={{...f,assigned,nL,rowH,y}};
    y+=rowH; return row;
  }});
  canvas.height=y+6;
}}
...
function draw(){{
  ...
  for(let h=0;h<=HOURS;h++){{
    ...
    ctx.fillStyle="#8a9ab5"; ctx.font="12px Arial"; ctx.textAlign="left";
    ctx.fillText(String(DAY_MIN+h).padStart(2,"0")+":00",x+3,HDR_H-8);
  }}
  ...
  layout.forEach((f,fi)=>{{
    ...
    // flight label (left column)
    ctx.fillStyle="#071b3a"; ctx.font="bold 13px Arial"; ctx.textAlign="right";
    ctx.fillText(f.short,LEFT-6,f.y+f.rowH/2-4);
    ctx.fillStyle="#8a9ab5"; ctx.font="11px Arial";
    ctx.fillText(f.dest||"",LEFT-6,f.y+f.rowH/2+12);
    ctx.textAlign="left";

    f.assigned.forEach(t=>{{
      const x1=LEFT+(t.sh-DAY_MIN)*HOUR_W;
      const bw=Math.max((t.eh-t.sh)*HOUR_W,8);
      const ly=f.y+ROW_PAD+t.lane*(LANE_H+LANE_PAD);
      const missing=t.worker.includes("❌");
      const isDragging=drag&&drag.fi===fi&&drag.task===t;

      ctx.globalAlpha=isDragging?0.3:0.92;
      ctx.fillStyle=missing?"#ffcccc":t.color;
      rr(ctx,x1,ly,bw,LANE_H,7); ctx.fill();
      ctx.globalAlpha=1;

      if(bw>24){{
        ctx.save();
        ctx.beginPath(); ctx.rect(x1+4,ly,bw-8,LANE_H); ctx.clip();
  ...
  if(drag&&drag.ghostX!==null){{
    const t=drag.task;
    const bw=Math.max((t.eh-t.sh)*HOUR_W,80);
    ctx.globalAlpha=0.85;
    ctx.fillStyle=t.color;
    rr(ctx,drag.ghostX-bw/2,drag.ghostY-LANE_H/2,bw,LANE_H,7); ctx.fill();
    ctx.globalAlpha=1;
    ctx.fillStyle="white"; ctx.font="bold 11px Arial"; ctx.textAlign="center";
    ctx.fillText(t.worker,drag.ghostX,drag.ghostY+LANE_H/2-10);
    ctx.textAlign="left";
  }}
}}
...
canvas.addEventListener("mousemove",e=>{{
  if(!drag)return;
  const{{px,py}}=canvasPos(e);
  drag.ghostX=px;drag.ghostY=py;draw();e.preventDefault();
}});
canvas.addEventListener("mouseup",e=>{{
  if(!drag)return;
  finishDrag(flightAtY(canvasPos(e).py));
}});
...
canvas.addEventListener("touchstart",e=>{{
  ...
  const hit=hitTest(px,py);
  if(hit){{
    drag={{...hit,ghostX:px,ghostY:py,startFi:hit.fi}};
    draw();
    e.preventDefault();   // stops page scroll only when on a block
  }}
}},{{passive:false}});
...
canvas.addEventListener("touchend",e=>{{
  ...
  const r=canvas.getBoundingClientRect();
  finishDrag(flightAtY(t.clientY-r.top));
}});
...
function finishDrag(targetFi){{
  ...
  const srcF=layout[src.startFi];
  const tgtF=layout[targetFi];
  const role=src.task.role;
  const missing=tgtF.assigned.find(t=>t.worker.includes("❌")&&t.role===role);

  if(!missing&&!tgtF.assigned.some(t=>t.role===role)){{
    info.className="err";info.style.display="block";
    info.textContent="❌ אין תפקיד מתאים ("+role+") בטיסה "+tgtF.short;
    draw();return;
  }}
  ...
  info.textContent="✅ "+workerName+" הועבר מ-"+srcF.short+" ל-"+tgtF.short+" ("+role+")";
  ...
  try{{window.parent.postMessage({{isStreamlitMessage:true,type:"streamlit:setComponentValue",args:{{value:payload}}}},"*");}}catch(e){{}}
  try{{if(window.Streamlit)window.Streamlit.setComponentValue(payload);}}catch(e){{}}
}}
...
buildLayout();draw();
                ...
                swap_result = _components.html(
                    gantt_html_g,
                    height=max(500, len(flights_g) * 65 + 140),
                    scrolling=False,
                )
