...
def extract_shift_range_from_text(value):
    """
    Extract shift range from any text, for example:
    ד. שלנ 02:00-09:30
    רצ 12:30 - 21:00
    14:00-01:30
    """
    ...
    return normalize_time_text(match.group(1)), normalize_time_text(match.group(2))


def extract_shift_range_from_row(row):
    """Search all cells in a row for a shift range."""
    for value in row.values:
        start, end = extract_shift_range_from_text(value)
        if start and end:
            return start, end
    return "", ""


def row_looks_like_shift_header(row, name_col):
    """
    A shift header row is usually a colored/category row that contains hours,
    not an actual worker row.
    """
    start, end = extract_shift_range_from_row(row)
    if not start or not end:
        return False

    name_value = clean_text(row.get(name_col, ""))
    # If the name cell itself contains a shift range, it is almost certainly a block title.
    if extract_shift_range_from_text(name_value) != ("", ""):
        return True

    # If there is no usable employee name but the row contains shift hours, it is a header.
    if name_value == "":
        return True
...
def normalize_employees(df):
    ...
    # Create shift columns if missing. They will be filled from block headers.
    ...
    # Read shift blocks from rows that contain ranges such as 02:00-09:30.
    for idx, row in df.iterrows():
        found_start, found_end = extract_shift_range_from_row(row)

        if found_start and found_end:
            current_shift_start = found_start
            current_shift_end = found_end

            if row_looks_like_shift_header(row, "שם"):
                rows_to_drop.append(idx)
                continue

        # If this is a normal employee row and no explicit shift exists, inherit current block shift.
        name_value = clean_text(row.get("שם", ""))
        if name_value and idx not in rows_to_drop:
            if clean_text(df.at[idx, "תחילת משמרת"]) == "" and current_shift_start:
                df.at[idx, "תחילת משמרת"] = current_shift_start
            if clean_text(df.at[idx, "סוף משמרת"]) == "" and current_shift_end:
                df.at[idx, "סוף משמרת"] = current_shift_end
    ...
    # Remove accidental block-title rows that survived.
    df = df[~df["שם"].astype(str).apply(lambda x: bool(extract_shift_range_from_text(x)[0]))].copy()
...
def employee_shift_text(employees_df, emp_name):
    """
    Returns exact shift hours assigned from the employee roster block headers.
    Example: 02:00-09:30 / 14:00-01:30 / 22:00-07:00
    """
...
with st.expander("🕒 בדיקת שעות משמרת שנקראו מקובץ העובדים"):
    preview_cols = ["שם", "תחילת משמרת", "סוף משמרת"]
    available_cols = [col for col in preview_cols if col in employees_df.columns]
    st.dataframe(employees_df[available_cols], use_container_width=True)
