...
def area_switch_penalty(assignments, emp, role):
    ...
    Goal:
    - Do not move workers between departure hall and check-in counters unless necessary.
    - Keep team leads mostly in departure hall.
    - Prefer employees already used in the same operational area.
    """
    emp_name = emp["שם"]
    target_area = role_area(role)
    history = employee_area_history(assignments, emp_name)

    # No history yet: choose by natural default area.
    if not history:
        default_area = default_area_for_employee(emp)
        if default_area == target_area:
            return 0
        return 6

    # Already working in this area: good.
    if target_area in history:
        return 0

    # Area switch: avoid unless no choice.
    return 20


def continuation_text_for_employee(timed_df, current_idx, emp, employees_df):
    """
    Text for where the employee goes after current task.
    Uses actual next flight if any.
    Otherwise uses preferred steady area:
    - Team leads: stay in departure hall.
    - Others: check-in counters.
    """
    ...
    if not emp_row.empty and str(emp_row.iloc[0].get("ראש צוות", "")).strip() == "כן":
        return "המשך אולם יציאה"
...
def build_next_task_labels(result_df, employees_df):
    """
    Adds operational continuity and real break/refresher reflection.

    Examples:
    נטע ונטוררו (ראש צוות) - הפסקה ו2365 דיילת
    רז טל (מפקח TSA) - 09 מפקח TSA
    דנה כהן (דיילת) - דלפקי צ׳ק אין
    """
    df = result_df.copy()
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        next_text = next_task_text_for_employee(timed_df, idx, emp, employees_df)

        emp_row = employees_df[employees_df["שם"] == emp]
        break_text = ""
        if not emp_row.empty:
            break_text = break_label_for_employee(emp_row.iloc[0])

        if break_text and not break_used.get(emp, False):
            break_used[emp] = True
            if next_text:
                next_text = f"{break_text} ו{next_text}"
            else:
                next_text = break_text
...
def build_counter_continuity_rows(result_labeled, employees_df):
    """
    Creates a per-employee operational summary:
    - If employee has no flight tasks: all shift at check-in counters / departure hall for team leads.
    - If employee has 1-2 flight tasks: reflect check-in counter continuation for agents.
    - Team leads are kept mostly in departure hall.
    """
    rows = []
    ...
    for _, emp in employees_df.iterrows():
        name = emp["שם"]
        shift = ""
        if clean_text(emp.get("תחילת משמרת", "")) and clean_text(emp.get("סוף משמרת", "")):
            shift = f'{emp.get("תחילת משמרת")}–{emp.get("סוף משמרת")}'
        ...
        if emp_tasks.empty:
            ...
            rows.append({
                ...
                "משמרת": shift,
                "טיסות": 0,
                "אזור מומלץ": default_area,
                "הערה": f"כל המשמרת ב{default_area}",
            })
            continue

        task_count = len(emp_tasks)
        roles = " | ".join(emp_tasks["תפקיד"].astype(str).tolist())
        flights = " | ".join(emp_tasks["טיסה"].astype(str).tolist())

        if str(emp.get("ראש צוות", "")).strip() == "כן":
            area_note = "אולם יציאה"
            comment = "ראש צוות נשאר באולם היציאה ככל האפשר"
        elif task_count <= 2:
            area_note = "דלפקי צ׳ק אין + טיסות בודדות"
            comment = "טיסה בודדת/שתיים, שאר המשמרת בדלפקי צ׳ק אין"
        else:
            area_note = "אולם יציאה"
            comment = "רצף טיסות, לא להחזיר לדלפקים באמצע אם אין צורך"

        rows.append({
            "עובד": name,
            "משמרת": shift,
            "טיסות": task_count,
            "טיסות משובצות": flights,
            "תפקידים": roles,
            "אזור מומלץ": area_note,
            "הערה": comment,
        })
...
def render_line(line):
    st.markdown(
        f'<div class="assignment-line {line_style(line)}">{safe_html(line)}</div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    ...
    '<div class="small-note">השלימי כאן גייט, רישוי, נוסעים, טרייני וסוג הכשרה. המנוע יתחשב גם במשמרות, הפסקות ורצף אזורי.</div></div>',
    unsafe_allow_html=True,
)
