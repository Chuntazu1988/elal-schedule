            ...
            if unassigned_df.empty:
                ...
                for shift_label, group in shift_groups:
                    ...
                    for _, r in group.iterrows():
                            ...
                            if not on_break and not break_done:
                                if st.button("▶ התחל הפסקה", key=f"brk_start_{btn_key}",
                                    ...
                                    st.session_state["break_log"][name] = {
                                        "start": _dt.now().strftime("%H:%M"),
                                        "ts":    int(_time.time()),   # Unix timestamp
                                        "end":   None
                                    }
                                    st.rerun()
                            elif on_break:
                        ...
                        # Show completed break times in local timezone
                        if break_done:
                            import streamlit.components.v1 as _comp_done
                            s_ts = break_info.get("ts", 0)
                            e_ts = break_info.get("end_ts", 0)
                            _comp_done.html(f"""<script>
(function(){{
  function fmt(ts){{
    if(!ts) return "?";
  ...
  var bs=document.getElementById("bstart_{btn_key}");
  var be=document.getElementById("bend_{btn_key}");
  if(bs) bs.textContent=fmt({s_ts});
  if(be) be.textContent=fmt({e_ts});
  // also try parent frame
  var pbs=window.parent.document.getElementById("bstart_{btn_key}");
  var pbe=window.parent.document.getElementById("bend_{btn_key}");
  if(pbs) pbs.textContent=fmt({s_ts});
  if(pbe) pbe.textContent=fmt({e_ts});
}})();
</script>""", height=0)

                        # Countdown timer — shown only while on break
                        if on_break:
                            import streamlit.components.v1 as _comp_brk
                            import time as _time_mod
                            start_ts = break_info.get("ts", int(_time_mod.time()))
                            timer_html = f"""
<div id="timer_{btn_key}" style="direction:rtl;font-family:Arial,sans-serif;
     padding:6px 14px;border-radius:8px;margin-top:4px;font-size:14px;font-weight:800;
     background:#fff8e1;border:1px solid #fbbf24;color:#92400e;">
  ⏱️ <span id="tlabel_{btn_key}">מחשב...</span>
</div>
<script>
(function(){{
  var startTs  = {start_ts};
  var duration = {break_duration} * 60;

  // Set local start time display
  ...
  var startEl = window.parent.document.getElementById("start_time_{btn_key}");
  ...
  function update() {{
    ...
    var el  = document.getElementById("tlabel_{btn_key}");
    var box = document.getElementById("timer_{btn_key}");
    if (!el) return;
    if (remaining <= 0) {{
      el.textContent = "⚠️ זמן ההפסקה הסתיים — חזרה לעבודה!";
      ...
      var s = remaining % 60;
      el.textContent = "נותרו " + m + ":" + String(s).padStart(2,"0") + " מתוך {break_duration} דק׳";
    }}
  }}
  update();
  setInterval(update, 1000);
                            ...
                            _comp_brk.html(timer_html, height=52)
