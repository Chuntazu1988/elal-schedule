...
def build_next_task_labels(result_df, employees_df):
    ...
    break_used = {}  # emp_name -> last break time in minutes (or -9999 if never)
    ...
    for idx, row in timed_df.sort_values("_start_dt").iterrows():
        ...
        last_break_m = break_used.get(emp, -9999)
        # Normalize task_end_m for overnight comparison
        te_norm_for_break = task_end_m
        if last_break_m > 0 and te_norm_for_break < last_break_m:
            te_norm_for_break += 1440
        time_since_last_break = te_norm_for_break - last_break_m if last_break_m != -9999 else 9999

        needs_break_now = (
            bool(break_text) and
            last_break_m == -9999  # never had a break yet
        )
        # Also needs break if 4+ hours since last break
        needs_another_break = (
            last_break_m != -9999 and
            time_since_last_break >= MIN_BETWEEN_BREAKS
        )
        ...
        # Night shift: check if break window (00:00-02:00) is approaching
        night_needs_break = False
        if shift_type == "night" and last_break_m == -9999:
            te_norm = task_end_m if task_end_m < 21 * 60 else task_end_m - 1440
            if te_norm <= NIGHT_BREAK_WINDOW_END:
                night_needs_break = True

        def mark_break():
            break_used[emp] = task_end_m

        if next_plain:
            if needs_break_now or needs_another_break:
                mark_break()
                next_text = f"{break_text or 'הפסקה'} ו{next_plain}"
            ...
            if needs_break_now or needs_another_break:
                mark_break()
                next_text = f"{break_text or 'הפסקה'} ו{return_text}"
        ...
        deadline_suffix = (
            ...
            if break_deadline and last_break_m == -9999
            else ""
        )
