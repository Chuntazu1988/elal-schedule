...
def build_schedule(flights_df, employees_df, current_time=None):
    ...
    current_time: datetime עם timezone (ישראל).
                  ...
                  אם לא הועבר — מתנהג כבעבר (כל הטיסות נכנסות).
    """
    # חישוב דקות נוכחיות (0–1439) פעם אחת מחוץ ללולאה
    ...
    for _, flight in flights_df.iterrows():
        ...
        if current_minutes is not None:
            try:
                ...
                # תיקון חציית חצות: אם ההפרש מוזר מאוד — מוסיפים יממה
                if diff < -720:
                    diff += 1440
                if diff < -15:
                    continue   # טיסה שעברה — לא נשבץ אליה
            except Exception:
                pass  # אם השעה לא ניתנת לפירוש — כוללים ברירת מחדל
