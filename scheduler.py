...
def is_available(assignments, emp_name, start, end, emp_row=None, role=None, flight_gate=None, flight_departure=None):
    ...
    # מפקח TSA: בדיקת מגבלת 4 טיסות חופפות באותה שלוחה
    if is_tsa_inspector and new_terminal:
        tsa_overlap = 0
        for t in assignments:
            if t.get("עובד") != emp_name or t.get("תפקיד בסיס") != "מפקח TSA":
                continue
            if get_terminal(t.get("_gate", "")) != new_terminal:
                continue
            ts_str = clean_text(t.get("התחלה", ""))
            te_str = clean_text(t.get("סיום",   ""))
            if not ts_str or not te_str:
                continue
            ts_dt = to_datetime_time(ts_str)
            te_dt = to_datetime_time(te_str)
            ts_m  = to_m(ts_dt)
            te_m  = to_m(te_dt)
    ...
    for task in assignments:
        ...
        if not (start_m >= ee_m + buf or end_m <= es_m - buf):
            if is_tsa_inspector and task.get("תפקיד בסיס") == "מפקח TSA":
                existing_terminal = get_terminal(task.get("_gate", ""))
                if new_terminal and existing_terminal and new_terminal == existing_terminal:
                    # בדיקת טווח שעתיים בין המראות
                    if flight_departure and is_time_text(flight_departure):
                        ex_dep = task.get("_departure", "")
                        if ex_dep and is_time_text(ex_dep):
                            new_dep_m = time_to_minutes(flight_departure)
                            ex_dep_m  = time_to_minutes(ex_dep)
                            diff = abs(new_dep_m - ex_dep_m)
                            if diff > 720: diff = 1440 - diff
                            if diff > 120:
                                return False  # מעל שעתיים — לא מאשרים
                    continue  # אותה שלוחה + טווח שעתיים — מאשרים
            return False
        ...
        for role in ROLE_ORDER:
            ...
            for i in range(amount):
                ...
                for _, emp in candidates.iterrows():
                    ...
                    if (
                        ...
                        and is_available(assignments, name, start, end, emp,
                                        ...
                                        flight_departure=clean_text(flight.get("המראה", "")))
                ...
                task = {
                    ...
                    "_departure":  clean_text(flight.get("המראה", "")),
                    "סיבה":        reason,
                }
