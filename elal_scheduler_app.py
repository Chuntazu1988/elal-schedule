...
st.markdown(
    ...
    .sub-title { color: #666; font-size: 16px; margin-top: 0; }
    </style>
    """,
...
st.markdown('<p class="sub-title">העלאת סידור יומי + עובדים → השלמת נתונים → שיבוץ → הורדת אקסל</p>', unsafe_allow_html=True)
...
ROLE_COLUMNS = ["ראש צוות", "דייל", "מתאם תורים", "מפקח TSA", "שומר TSA", "חונך רצים", "מסמיך רצים", "טרייני רצ"]
...
def load_daily_schedule(uploaded_file):
    ...
    for _, row in raw.iterrows():
        ...
        flights.append({
            ...
            "זמנים": "" if pd.isna(time_text) else str(time_text).strip(),
            "המראה": departure,
            "בורדינג": boarding,
            "גייט": "",
...
def normalize_employees(employees_df):
    ...
    for old, new in aliases.items():
        if old in employees_df.columns:
            if new not in employees_df.columns:
...
def get_requirements(flight):
    ...
    req = {"ראש צוות": 1, "דייל": 1, "מתאם תורים": 0, "מפקח TSA": 0, "שומר TSA": 0}
...
def role_start_time(flight, role):
    ...
    body = get_body_type(flight)
    remote_narrow = body == "צר גוף" and str(flight.get("גייט", "")).upper().strip() in REMOTE_GATES
...
def assigned_minutes(assignments, emp_name):
    ...
    for task in assignments:
        if task["עובד"] != emp_name:
            continue
        total += int((to_datetime_time(task["סיום"]) - to_datetime_time(task["התחלה"])).total_seconds() / 60)
    return total
...
def count_team_lead_tasks(assignments, emp_name):
    return sum(1 for task in assignments if task["עובד"] == emp_name and str(task["תפקיד"]).startswith("ראש צוות"))


def sort_candidates(candidates, assignments, role):
    candidates = candidates.copy()
    if role == "ראש צוות":
        candidates["_score"] = candidates["שם"].apply(lambda n: count_team_lead_tasks(assignments, n))
        return candidates.sort_values("_score")
    if role == "דייל":
        candidates["_score"] = candidates.apply(lambda r: 0 if str(r.get("ראש צוות", "")).strip() == "כן" else 1, axis=1)
        return candidates.sort_values("_score")
    return candidates
...
def build_schedule(flights_df, employees_df):
    assignments = []
    role_order = ["ראש צוות", "דייל", "מתאם תורים", "מפקח TSA", "שומר TSA"]
        ...
        for role in role_order:
            ...
            for i in range(amount):
                ...
                for _, emp in candidates.iterrows():
                    name = emp["שם"]
                    if is_within_shift(emp, start, end) and is_available(assignments, name, start, end) and has_room_for_break(assignments, emp, name, start, end):
                        selected = name
                        break
...
def build_next_task_labels(result_df, employees_df):
    df = result_df.copy()
    df["_start_dt"] = pd.to_datetime(df["התחלה"], format="%H:%M", errors="coerce")
    break_used = {}
    labels = {}

    for idx, row in df.sort_values("_start_dt").iterrows():
        ...
        future = df[(df["עובד"].astype(str) == emp) & (df["_start_dt"] > row["_start_dt"])].sort_values("_start_dt")
    ...
    df["טקסט עובד"] = pd.Series(labels)
    return df.drop(columns=["_start_dt"])


def build_workload(result_df, employees_df):
    rows = []
    real_workers = sorted([w for w in result_df["עובד"].dropna().unique() if "❌" not in str(w)])
    for emp in real_workers:
        tasks = result_df[result_df["עובד"] == emp]
        total = 0
        for _, task in tasks.iterrows():
            total += int((to_datetime_time(task["סיום"]) - to_datetime_time(task["התחלה"])).total_seconds() / 60)
        emp_row = employees_df[employees_df["שם"] == emp]
        break_min = required_break(emp_row.iloc[0]) if not emp_row.empty else 0
        rows.append({"עובד": emp, "משימות": len(tasks), "דקות עבודה": total, "הפסקה נדרשת": break_min, "סה״כ כולל הפסקות": total + break_min})
...
def build_output_table(flights_df, result_labeled):
    ...
    for _, flight in flights_df.iterrows():
        ...
        for _, task in tasks.iterrows():
            ...
            elif "דייל" in role or "שומר TSA" in role:
...
def to_excel_bytes(output_df, workload_df):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for col, width in {"A": 16, "B": 10, "C": 18, "D": 18, "E": 60, "F": 60}.items():
...
edited_flights = st.data_editor(flights_df, use_container_width=True, num_rows="dynamic", key="flights_editor")
