...
st.title("✈️ מערכת שיבוץ טיסות")
st.caption("גרסה מלאה עם חוקים: TSA, דיילים, ראשי צוותים, טרייני, משמרות, חפיפות, הפסקות, חוסרים ועומס")
...
def shift_length(emp):
    start = clean_text(emp.get("תחילת משמרת", ""))
    end = clean_text(emp.get("סוף משמרת", ""))
...
def is_available(assignments, emp_name, start, end):
    ...
    for task in assignments:
        ...
        old_start = to_datetime_time(task["התחלה"])
        old_end = to_datetime_time(task["סיום"])

        if not (start >= old_end + buffer or end <= old_start - buffer):
            return False

    return True


def count_role(assignments, emp_name, startswith_text):
    return sum(
        1 for task in assignments
        if task["עובד"] == emp_name and str(task["תפקיד"]).startswith(startswith_text)
    )


def count_all(assignments, emp_name):
...
def sort_candidates(candidates, assignments, role):
    ...
    if role == "ראש צוות":
        candidates["_score"] = candidates["שם"].apply(lambda name: count_role(assignments, name, "ראש צוות"))
    ...
    if role == "דייל":
        ...
        candidates["_score2"] = candidates["שם"].apply(lambda name: count_all(assignments, name))
        return candidates.sort_values(["_score1", "_score2"])

    candidates["_score"] = candidates["שם"].apply(lambda name: count_all(assignments, name))
    return candidates.sort_values("_score")
...
st.caption("השלימי כאן גייט, רישוי, נוסעים, טרייני וסוג הכשרה. זה משפיע על החוקים.")
...
if st.button("🚀 בנה שיבוץ", use_container_width=True):
    try:
        ...
        with tab_schedule:
            ...
            for _, row in display_df.iterrows():
                with st.container(border=True):
                    st.markdown(f"### ✈️ {row['מספר טיסה']} → {row['יעד']}")
                    st.caption(f"🕒 {row['זמנים']} | 🛩️ {str(row['מטוס/רישוי']).replace(chr(10), ' / ')}")

                    c1, c2 = st.columns(2)

                    with c1:
                        st.markdown("#### 👔 ראש צוות / מתאם / TSA")
                        text = str(row["ראש צוות / מתאם תורים / מפקח TSA"])
        ...
        st.download_button(
            "⬇️ הורדת אקסל מלא",
            data=excel_data,
            file_name="SCHEDULE_PROFESSIONAL_FULL.xlsx",
